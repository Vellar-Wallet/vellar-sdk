...
higher ceiling (self-hosted, or a hosted one that allows it). A plain transfer
stays well under the default.

## Errors

The client throws typed errors (all extend `VellarError` — see
[Errors](./errors.md)):
...
| `X402PaymentError` | The payment couldn't be built or sent locally (simulation, expired auth entry, non-replayable body). |
| `X402SigningError` (extends `X402PaymentError`) | The signer was misconfigured, or asked to sign an entry it can't. |
