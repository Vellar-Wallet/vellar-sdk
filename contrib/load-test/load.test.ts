...
//
// Simulates many concurrent payment submissions through payments-client (the
// payment flow behind payments.ts). It drives `createPaymentClient` with a fake
// kit / sac and a backend whose latency and error rate are configurable, then
// measures latency and error rate at increasing concurrency levels and prints a
// report. This is an OPTIONAL load test — it does not run in `npm test`. Run it
// deliberately with `npm run test:load` (optionally as a CI job).
//
// Bottleneck note: the SDK submits each payment as a fully-async, shared-nothing
// promise chain, so on a single Node process there is no in-process serialization —
// throughput scales with concurrency up to the transport the backend represents.
// The bottleneck under load is the backend / relayer round-trip, modeled here by
// `BACKEND_LATENCY_MS`; `failEveryN` models deterministic transport failures.
//
...
function makeBackend(opts?: { failEveryN?: number; latencyMs?: number }): {
...
describe("payments.ts concurrent submission load test (contrib)", () => {
  it(
